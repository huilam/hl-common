...
public class CannyEdgeDetector {
	...
	{
		String sImgPath = new File(".").getCanonicalPath()+"/test";
		String sImgFileName = sImgPath+"/CAM_05_empty.png";
		
		BufferedImage imgOrig = ImgUtil.loadImage(sImgFileName);
		imgOrig = ImgUtil.grayscale(imgOrig);
		int iNewWidth 	= 640;
		imgOrig = ImgUtil.resizeImg(imgOrig, iNewWidth, 0, true);
		
		CannyEdgeDetector filter = new CannyEdgeDetector();
		float thresholdLow = 0.6f;
		filter.setLowThreshold(thresholdLow);
		filter.setHighThreshold(1f);
		filter.setSourceImage(imgOrig);
		filter.process();
		BufferedImage imgEdges = filter.getEdgesImage();	
		
		ImgUtil.saveAsFile(imgEdges, new File(sImgFileName+"_edges_"+thresholdLow+".png"));
	}
